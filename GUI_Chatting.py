...
def create_gui():
    ...
    #appearance_mode_switch.pack(side="left", padx=0)
    appearance_mode_switch.place(x=1320, y=130)
    ...
    agent_optionmenu = ctk.CTkOptionMenu(agent_frame, values=["Plant", "Algae", "Fungi", "Bacteria"], command=select_biological_agent, font=("Raleway", 12))
    agent_optionmenu.pack(side="left", padx=10)
    agent_optionmenu.set("Plant")  # Default to "Plant"


    # Create the chat history frame
    chat_frame = ctk.CTkFrame(window)
    chat_frame.pack(pady=10, padx=10, fill="both", expand=True)

    # Create the scrollable frame for the chat history
    chat_scrollable_frame = ctk.CTkScrollableFrame(chat_frame)
    chat_scrollable_frame.pack(pady=10, padx=10, fill="both", expand=True)

    # Create the prompt frame
    prompt_frame = ctk.CTkFrame(window)
    prompt_frame.pack(pady=10, padx=10, fill="x")

    # Create the prompt entry widget
    prompt_entry = ctk.CTkEntry(prompt_frame, width=620, font=("Raleway", 12), placeholder_text="Enter new data...")
    prompt_entry.pack(side="left", padx=10, pady=5, fill="x", expand=True)

    # Create the send button
    send_button = ctk.CTkButton(prompt_frame, text="Analyze", command=send_message, font=("Raleway", 12, "bold"))
    send_button.pack(side="left", padx=10, pady=5)

    # Create the loading label
    loading_label = ctk.CTkLabel(prompt_frame, text="Loading...", font=("Raleway", 12, "bold"))
